...
#include <stdio.h>
#include <math.h>
#include "../../../../../tools/ed2_compat_prefix.h"
#ifdef CRAY
#include <stdlib.h>
#endif
...
int rams_c_read(int *fbyte,int *numbytes,int *a)
{
  ...
  retcode=fseek(ramsfile,*fbyte,0);
  if (fread(a,1,*numbytes,ramsfile) != (size_t)*numbytes) return -1;
  return(retcode);
}
...
void vfirecr(int *unit,float *a,int *n,char *type,char *b,int *irec)    
{
  ...
  if (fread(b,1,80,ramsfile) != 80) return;
  sscanf(b,"%d %d %f %f",&nn, &nbits, &bias, &fact);
  inverse_fact = 1./fact;
  nchs=nbits/6;
  if (fread(b,1,*n * nchs,ramsfile) != (size_t)(*n * nchs)) return;
  if( nn != *n )
