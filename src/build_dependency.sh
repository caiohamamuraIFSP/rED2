...
SOURCES=$(echo $SOURCES | sed 's/ED2\/ED\/src\/driver\/edmain.F90//')

echo SOURCES = $SOURCES ED2/ED/src/utils/utils_c.c > sources.mk
...
for src in $SOURCES; do
  ...
  MODS_USED=$(for mod in $MODS_USED; do
    if [ -f ED2/ED/src/*/$mod.F90 -o -f ED2/ED/src/*/$mod.f90 ]; then
      echo $mod
    fi
  done)
